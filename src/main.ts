  ...
  node.addEventListener("click", () => {
    if (registerStep === 1) {
      ...
      if (nickname.length < 3) {
        showMessage(registerMessage, "Nickname too short.");
        return;
      }
    ...
    if (registerStep === 2) {
      ...
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        showMessage(registerMessage, "Invalid email.");
        return;
      }
...
forgotForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (data?.debug_token) {
      ...
      showMessage(resetMessage, "Debug token filled.", true);
    ...
    showMessage(
      forgotMessage,
      "If this email exists, recovery has been sent.",
      true
    );
  } catch (error) {
    showMessage(
